...
export default function Recommendation({ name, youtubeLink, score, id, onUpvote = () => 0, onDownvote = () => 0 }) {
  ...
  return (
    <Container>
      <Row>{name}</Row>
      <ReactPlayer id='play' url={youtubeLink} width="100%" height="100%" />
      <Row>
        <GoArrowUp size="24px" id="up" onClick={handleUpvote} />
        {score}
        <GoArrowDown size="24px" id="down" onClick={handleDownvote} />
      </Row>
    </Container>
  );
}
